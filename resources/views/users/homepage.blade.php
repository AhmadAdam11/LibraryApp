...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-4xl font-bold text-gray-800">Book List</h2>
        <p class="text-l text-gray-500 mt-0.5">Find Your Favorit Book -></p>
    </div>
</div>
...
<form method="GET" id="category-form">
    <div class="flex gap-2 flex-wrap mb-6">
        ...
        <button type="submit" name="category" value=""
            ...
            {{ !$categoryId ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-gray-800 text-gray-400 border-gray-700 hover:border-indigo-500 hover:text-gray-200' }}">
            All
        </button>

        @foreach ($categories as $cat)
            <button type="submit" name="category" value="{{ $cat->id }}"
                class="px-4 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200
                {{ $categoryId == $cat->id ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-gray-800 text-gray-400 border-gray-700 hover:border-indigo-500 hover:text-gray-200' }}">
                {{ $cat->name }}
            </button>
        @endforeach
